...
// ============================================================
// COMBAT (Slice 5a) — handlers + engine inline
// ============================================================
// El engine es una copia inline del modulo standalone server/combat_engine.js
// con prefijo "combat" en los simbolos top-level para no chocar con el resto
// del worker (GE, etc). Pegado tal cual del archivo de referencia.
// ============================================================
...
const COMBAT_XP_PER_DMG_PER_SKILL = 4 / 3;
...
function combatAwardXp(stats, damage) {
  if (damage <= 0) return { attack: 0, strength: 0, defence: 0, hp: 0 };
  const each = Math.floor(damage * COMBAT_XP_PER_DMG_PER_SKILL);
  stats.attack_xp += each;
  stats.strength_xp += each;
  stats.defence_xp += each;
  stats.hp_xp += each;
  return { attack: each, strength: each, defence: each, hp: each };
}
...
async function combatAttackNpc(db, userId, npcInstanceId, opts) {
  ...
  const xpGained = combatAwardXp(stats, dmgToNpc);
