...
const COMBAT_RANGE_TOLERANCE = 3.5;   // Margen sobre attack_range. Cubre los
                                       // ~3m del patrol radius visual + holgura.
...
async function combatReviveExpiredNpcs(db, opts) {
  ...
  // Cada instance puede tener su propio spawn point (npc_instances.spawn_x/z).
  // Si esa columna está NULL, fallback al spawn point del def (legacy).
  // Esto evita que múltiples instancias del mismo def aparezcan amontonadas.
  const result = await db.run(
    `UPDATE npc_instances
     SET status = 0,
         ...
         in_combat_with = NULL,
         last_attack_at = NULL,
         x = COALESCE(npc_instances.spawn_x, (SELECT spawn_x FROM npc_defs WHERE id = npc_instances.def_id)),
         z = COALESCE(npc_instances.spawn_z, (SELECT spawn_z FROM npc_defs WHERE id = npc_instances.def_id))
     WHERE status = 1
...
async function combatAttackNpc(db, userId, npcInstanceId, opts) {
  ...
  if (npcKilled) {
    try {
      ...
      // No romper el ataque si falla el loot (p. ej. tabla vacía o
      // npc sin entradas). Solo lo logueamos.
