/**
 * SebasPresent — Auth + Position + Inventory + Bank + GE Worker (Slice 4c)
 *
 * Endpoints:
 ...
 *
 *   NEW in Slice 4c (Grand Exchange):
 *   GET  /api/ge/orders              → { slots: [orders...], maxSlots: 8 }
 *      Tus 8 slots de orden con su progreso.
 *   POST /api/ge/place   { item_id, side, price, qty } → { orderId }
 *      side: "buy" o "sell". Coins/items salen de user_bank al escrow.
 *      Valida banda de precio (+/-5%) y slots disponibles (max 8).
 *   POST /api/ge/cancel  { order_id } → { ok: true }
 *      Cancela orden propia y devuelve escrow remanente.
 *   GET  /api/ge/item/:id            → info de mercado del item
 *      { item, guide_price, suggested_price, best_buy, best_sell, band: {min,max} }
 *   GET  /api/ge/item/:id/history?days=7 → puntos para grafico
 *      { points: [{matched_at, matched_price, qty}, ...] }
 *   GET  /api/ge/search?q=X          → busqueda de items
 *      { items: [{id, name, icon, base_price, suggested_price}, ...] }
 *
 ...
 *
 * NOTA: el engine del GE (matching, escrow, validacion) esta inline
 * al final de este archivo en la seccion "GE ENGINE". Esta tambien
 * disponible como modulo aparte en server/ge_engine.js para tests
 * y como referencia, pero esta version inline es la canonical para
 * deploy en Cloudflare Workers sin bundler.
 */
...
const GE_GHOST_TIMESTAMP_FAR_FUTURE = 9_999_999_999_999; // año 2286, hace fallback a fantasmas
...
/**
 * GET /api/ge/orders
 * Devuelve las ordenes abiertas del usuario (max 8). Tambien
 * incluye las completadas/canceladas recientes para que la UI
 * pueda mostrar "tu orden de logs se completo".
 */
async function handleGeGetOrders(request, env) {
  ...
  const open = await db.all(
    `SELECT o.id, o.item_id, o.side, o.price, o.qty_total, o.qty_filled,
            ...
            o.coins_recovered, o.created_at, o.completed_at,
            i.name, i.icon, i.stackable
     FROM ge_orders o
     JOIN items i ON i.id = o.item_id
     WHERE o.user_id = ? AND o.status = ?
     ORDER BY o.created_at ASC`,
    [session.user_id, GE_STATUS_OPEN]
  );

  // Cerradas recientes (ultimas 24h, para notificaciones de "completed")
  ...
  const recent = await db.all(
    `SELECT o.id, o.item_id, o.side, o.price, o.qty_total, o.qty_filled,
     ...
     FROM ge_orders o
     JOIN items i ON i.id = o.item_id
     WHERE o.user_id = ? AND o.status IN (?, ?) AND o.completed_at >= ?
     ORDER BY o.completed_at DESC
     LIMIT 20`,
    [session.user_id, GE_STATUS_COMPLETED, GE_STATUS_CANCELLED, since]
  );
...
// ============================================================
// GE ENGINE (inline en worker.js para deploy sin bundler)
// ============================================================
// CANONICAL: este bloque DEBE coincidir 1:1 con server/ge_engine.js
// del repo. Si tocas uno, toca el otro. Los tests viven en
// tests/test_ge_engine.py y replican la logica en Python sobre
// SQLite local (118 asserts, 0 fails al cierre de sesion A).
//
// INVARIANTES:
...
async function gePlaceOrder(db, userId, { itemId, side, price, qty }) {
  ...
  if (side === GE_SIDE_BUY) {
    ...
    const coinsRow = await db.first(
      'SELECT slot_index, quantity FROM user_bank WHERE user_id = ? AND item_id = ?',
      [userId, GE_COIN_ITEM_ID]
    );
    if (!coinsRow || coinsRow.quantity < cost) throw geMakeErr('insufficient_coins');

    if (coinsRow.quantity === cost) {
      stmts.push({
        sql: 'DELETE FROM user_bank WHERE user_id = ? AND item_id = ?',
        params: [userId, GE_COIN_ITEM_ID],
      });
    } else {
      stmts.push({
        sql: 'UPDATE user_bank SET quantity = quantity - ?, updated_at = ? WHERE user_id = ? AND item_id = ?',
        params: [cost, now, userId, GE_COIN_ITEM_ID],
      });
    }
    ...
    stmts.push({
      sql: `INSERT INTO ge_orders
            ...
            VALUES (?, ?, ?, ?, ?, 0, ?, ?, 0, 0, 0, ?)`,
      params: [userId, itemId, GE_SIDE_BUY, price, qty, GE_STATUS_OPEN, cost, now],
    });
  ...
  if (!itemRow || itemRow.quantity < qty) throw geMakeErr('insufficient_items');
  ...
  stmts.push({
    sql: `INSERT INTO ge_orders
            (user_id, item_id, side, price, qty_total, qty_filled, status,
             coin_escrow, item_escrow, avg_fill_price, coins_recovered, created_at)
          VALUES (?, ?, ?, ?, ?, 0, ?, 0, ?, 0, 0, ?)`,
    params: [userId, itemId, GE_SIDE_SELL, price, qty, GE_STATUS_OPEN, qty, now],
  });
  ...
  const row = await db.first(
    `SELECT id FROM ge_orders WHERE user_id = ? AND created_at = ? AND item_id = ? AND side = ?
     ORDER BY id DESC LIMIT 1`,
    [userId, now, itemId, GE_SIDE_SELL]
  );
...
async function geCancelOrder(db, userId, orderId) {
  ...
  if (userId !== GE_SYSTEM_USER_ID) {
    const bankState = await geLoadBankState(db, userId);
    if (order.side === GE_SIDE_BUY && order.coin_escrow > 0) {
      geAddBankDeposit(stmts, bankState, userId, GE_COIN_ITEM_ID, order.coin_escrow, now);
    } else if (order.side === GE_SIDE_SELL && order.item_escrow > 0) {
      geAddBankDeposit(stmts, bankState, userId, order.item_id, order.item_escrow, now);
    }
  }

  stmts.push({
    sql: `UPDATE ge_orders SET status = ?, coin_escrow = 0, item_escrow = 0, completed_at = ? WHERE id = ?`,
...
async function geRunMatcher(db) {
  ...
  for (const row of items) {
    const n = await geMatchItem(db, row.item_id);
    if (n > 0) { total += n; touched.push(row.item_id); }
  }
...
async function geApplyMatch(db, buy, sell, qty, matchPrice) {
  ...
  let buyerBank = null, sellerBank = null;
  if (buy.user_id !== GE_SYSTEM_USER_ID) buyerBank = await geLoadBankState(db, buy.user_id);
  if (sell.user_id !== GE_SYSTEM_USER_ID) {
    sellerBank = (sell.user_id === buy.user_id) ? buyerBank : await geLoadBankState(db, sell.user_id);
  }
  ...
  stmts.push({
    sql: `UPDATE ge_orders SET qty_filled = ?, coin_escrow = ?, status = ?, completed_at = ?,
            avg_fill_price = ?, coins_recovered = ? WHERE id = ?`,
    params: [newBuyFilled, newBuyEscrow, newBuyStatus, newBuyCompletedAt,
             newBuyAvg, newBuyRecovered, buy.id],
  });

  if (buyerBank) {
    geAddBankDeposit(stmts, buyerBank, buy.user_id, buy.item_id, qty, now);
    if (refund > 0) geAddBankDeposit(stmts, buyerBank, buy.user_id, GE_COIN_ITEM_ID, refund, now);
  }
  ...
  stmts.push({
    sql: `UPDATE ge_orders SET qty_filled = ?, item_escrow = ?, status = ?, completed_at = ?,
  ...
  if (sellerBank) {
    geAddBankDeposit(stmts, sellerBank, sell.user_id, GE_COIN_ITEM_ID, buyerSpend, now);
  }

  stmts.push({
    sql: `INSERT INTO ge_history (item_id, buy_order_id, sell_order_id, buyer_id, seller_id,
            matched_price, qty, matched_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    params: [buy.item_id, buy.id, sell.id, buy.user_id, sell.user_id, matchPrice, qty, now],
  });
