/**
 * SebasPresent — Auth + Position Worker (Slice 3)
 *
 * Endpoints:
 ...
 * Position: stored in users.last_x, users.last_z (REAL columns).
 *           Bounded to ±2048 (world half-size) to prevent abuse.
 */
...
// World bounds — must match WORLD_HALF in client/src/world.js
...
// ---------- NEW: Position handlers (Slice 3) ----------

/**
 * GET /api/position — returns the user's last saved world position.
 * Returns { x: 0, z: 0 } if never saved (new user spawns at origin).
 */
async function handleGetPosition(request, env) {
  ...
  // null-safe defaults: if column was just added by migration, value may be null
...
/**
 * POST /api/position — saves the user's current world position.
 * Body: { x: number, z: number }
 * Server clamps to world bounds to prevent invalid data.
 */
async function handleSavePosition(request, env) {
  ...
  // Clamp to world bounds (defense in depth — client also clamps)
