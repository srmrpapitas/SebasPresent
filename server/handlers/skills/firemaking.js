/**
 * SebasPresent — Firemaking handler (Sesión 30 + S32 reorg)
 *
 * Endpoint:
...
export async function handleFiremakingLight(request, env) {
  ...
  // 3) Tinderbox (inv o equip — usamos helper compartido)
  if (!(await hasItemAvailable(env, userId, 'tinderbox', { checkWeaponSlot: false }))) {
    return json({ error: 'no_tinderbox', message: 'Necesitas un yesquero.' }, 400);
  }
  ...
  // Sesión 30 — Spawneamos el fuego 1.2m DELANTE del player en dirección yaw.
