/**
 * SebasPresent — World module (Slice 2.8 — bounded continent)
 *
 * 4096×4096m continent with:
 *   - Hardcoded biome regions (deterministic, not pure noise)
 *   - 20 named places: 8 cities, 6 villages, 3 landmarks, 3 wilderness sites
 *   - Western quarter (x < -1024) is the Wilderness (PVP zone) — reddish
 *     ground tint, dead trees, warning banner on entry
 *   - Ocean visible past world bounds, player clamped to bounds
 *   - HUD: current region name, big banner on transitions
 *
 * The chunk system stays (only ~50 chunks rendered at once) so terrain
 * memory is bounded regardless of total world size.
 */
...
const WORLD_HALF     = 2048;       // half-size → world is 4096×4096
const WILDERNESS_X   = -1024;      // west of this is PVP
...
const PALETTE = {
  sky:        0x9ec0d6,
  fog:        0xa8c4d8,
  skyWild:    0x6a4040,           // sky tint when in wilderness
  fogWild:    0x6a3838,
  ocean:      0x4a7896,
  player:     0xc04a3a,
  marker:     0xfff04a,
...
const TREE_TYPE_DEFS = {
  normal: {
    name: 'Árbol',  chopLevel: 1,  xpReward: 25,  logItem: 'logs',
    trunkColor: 0x6b4423, canopyColor: 0x4a7a2a,
    trunkScale: 1.0, height: 2.5, canopyShape: 'sphere', canopyRadius: 1.4,
  },
  oak: {
    name: 'Roble', chopLevel: 15, xpReward: 37, logItem: 'oak_logs',
    trunkColor: 0x5a3618, canopyColor: 0x3d6420,
    trunkScale: 1.3, height: 3.4, canopyShape: 'sphere', canopyRadius: 2.0,
  },
  palm: {
    name: 'Palmera', chopLevel: 20, xpReward: 35, logItem: 'palm_logs',
    trunkColor: 0x8b6c3c, canopyColor: 0x88a838,
    trunkScale: 0.7, height: 4.5, canopyShape: 'flat', canopyRadius: 2.2,
  },
  pine: {
    name: 'Pino', chopLevel: 30, xpReward: 65, logItem: 'pine_logs',
    trunkColor: 0x4a2a14, canopyColor: 0x2a5028,
    trunkScale: 0.85, height: 4.2, canopyShape: 'cone', canopyRadius: 1.4,
  },
  maple: {
    name: 'Arce', chopLevel: 45, xpReward: 100, logItem: 'maple_logs',
    trunkColor: 0x6a4528, canopyColor: 0xb04826,
    trunkScale: 1.1, height: 3.6, canopyShape: 'sphere', canopyRadius: 1.9,
  },
  mahogany: {
    name: 'Caoba', chopLevel: 50, xpReward: 125, logItem: 'mahogany_logs',
    trunkColor: 0x5a2818, canopyColor: 0x2a5a1c,
    trunkScale: 1.4, height: 4.0, canopyShape: 'sphere', canopyRadius: 2.2,
  },
  yew: {
    name: 'Tejo', chopLevel: 60, xpReward: 175, logItem: 'yew_logs',
    trunkColor: 0x2a1a10, canopyColor: 0x1a4438,
    trunkScale: 1.6, height: 4.2, canopyShape: 'sphere', canopyRadius: 2.3,
  },
  magic: {
    name: 'Árbol Mágico', chopLevel: 75, xpReward: 250, logItem: 'magic_logs',
    trunkColor: 0xa8b8d0, canopyColor: 0x88ddff,
    trunkScale: 1.2, height: 4.5, canopyShape: 'crystal', canopyRadius: 2.0,
    emissive: 0x4488ff, emissiveIntensity: 0.45,
  },
  dead: {
    name: 'Árbol Muerto', chopLevel: 1, xpReward: 12, logItem: 'dead_logs',
    trunkColor: 0x3a2818, canopyColor: 0x4a3828,
    trunkScale: 0.9, height: 3.2, canopyShape: 'crystal', canopyRadius: 1.0,
  },
};
...
// ============================================================
//
// type drives the structure shape:
//   city     → 1 big central + 4 satellite pillars in a plaza
//   village  → 1 medium pillar + 2 small
//   tower    → 1 very tall thin pillar
//   mine     → low angular box structure
//   temple   → wide flat base + glowing white pillar
//   ruins    → 3 broken short pillars
//   altar    → low stone slab + dark crystal
//   boss     → tall blood-red obelisk
...
// Sorted by chunk for efficient per-chunk lookup
...
/**
 * Region-based biome lookup. Hardcoded zones with noise displacement
 * for organic boundaries (so transitions aren't straight lines).
 */
function biomeAt(x, z) {
  // Wilderness covers the entire western quarter
  if (x < WILDERNESS_X) return BIOMES.wilderness;

  // Central plaza around spawn
  if (Math.hypot(x, z) < 80) return BIOMES.plaza;

  // Noise displacement → organic biome borders, not straight lines
  const nx = (noise2d(x * 0.008, z * 0.008) - 0.5) * 90;
  const nz = (noise2d(x * 0.009 + 50, z * 0.009 + 50) - 0.5) * 90;
  const ex = x + nx;
  const ez = z + nz;

  // Snow / tundra in far north
  if (ez < -1300) return BIOMES.snow;
  // Beach along southern coast
  if (ez > 1450) return BIOMES.beach;
  // Desert in east strip
  if (ex > 900 && ez > -700 && ez < 700) return BIOMES.desert;
  // Jungle in south-east
  if (ez > 700 && ex > 200) return BIOMES.jungle;
  // Forest in north-center
...
/**
 * Return the name and type of the region the player is currently in.
 * Priority: named place > wilderness depth > biome.
 */
function getRegionInfo(x, z) {
  // Check proximity to named places
  for (const p of PLACES) {
    const r = p.type === 'city' ? 130 : p.type === 'village' ? 80 : 60;
    if (Math.hypot(p.x - x, p.z - z) < r) {
      const isWild = p.biome === 'wilderness';
      return { name: p.name, type: p.type, isWild, isPlace: true };
    }
  }

  // Wilderness depth levels
  if (x < WILDERNESS_X) {
...
function setupOcean() {
  // Giant blue plane covering everywhere — visible past world bounds
  // and through any gaps. Sits 0.4m below land.
...
function initTreeGeometries() {
  ...
  for (const [id, def] of Object.entries(TREE_TYPE_DEFS)) {
    ...
    const trunkGeom = new THREE.CylinderGeometry(
      trunkRadius * 0.8, trunkRadius * 1.1, def.height, 6
    );
    trunkGeom.translate(0, def.height / 2, 0);
    const trunkMat = new THREE.MeshLambertMaterial({
      color: def.trunkColor, flatShading: true,
    });

    let canopyGeom;
    if (def.canopyShape === 'sphere') {
      canopyGeom = new THREE.IcosahedronGeometry(def.canopyRadius, 0);
    } else if (def.canopyShape === 'cone') {
      canopyGeom = new THREE.ConeGeometry(def.canopyRadius, def.canopyRadius * 2.4, 7);
    } else if (def.canopyShape === 'flat') {
      canopyGeom = new THREE.IcosahedronGeometry(def.canopyRadius, 0);
      canopyGeom.scale(1, 0.35, 1);
    } else if (def.canopyShape === 'crystal') {
      canopyGeom = new THREE.OctahedronGeometry(def.canopyRadius, 0);
    } else {
      canopyGeom = new THREE.IcosahedronGeometry(def.canopyRadius, 0);
    }
...
function setupPlayer() {
  ...
  const mat = new THREE.MeshLambertMaterial({
    color: PALETTE.player, flatShading: true,
  });
...
function setupMarker() {
  ...
  const mat = new THREE.MeshBasicMaterial({
    color: PALETTE.marker, transparent: true, opacity: 0.9, side: THREE.DoubleSide,
  });
...
function buildPlaceStructure(place) {
  ...
  switch (place.type) {
    case 'city': {
      // 1 large central pillar (10m) + 4 satellite pillars (6m) in a square
    ...
    case 'village': {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const sx = Math.cos(a) * 2.8;
        const sz = Math.sin(a) * 2.8;
        const sat = new THREE.Group();
        sat.position.set(sx, 0, sz);
    ...
    case 'tower': {
      ...
      // Conical roof
      const roofGeom = new THREE.ConeGeometry(1.2, 2.5, 8);
    ...
    case 'mine': {
      // Low angular dark structure
      const baseG = new THREE.BoxGeometry(4, 1.5, 4);
      const base = new THREE.Mesh(baseG, stoneMat);
      base.position.y = 0.75;
      group.add(base);
      // Entrance (dark rectangle on one side)
      const entryG = new THREE.BoxGeometry(1.2, 1.6, 0.4);
      const entryM = new THREE.MeshLambertMaterial({ color: 0x101010 });
      const entry = new THREE.Mesh(entryG, entryM);
      entry.position.set(0, 0.85, 2.0);
      group.add(entry);
      // Crystal stuck on top
    ...
    case 'temple': {
      // Wide round base + tall pale pillar with glowing top
      const baseG = new THREE.CylinderGeometry(3, 3.3, 0.6, 12);
      const base = new THREE.Mesh(baseG, new THREE.MeshLambertMaterial({
        color: 0xc8c0b0, flatShading: true,
      }));
      base.position.y = 0.3;
      group.add(base);
      group.add(makeColumn(8, 0.6, new THREE.MeshLambertMaterial({
        color: 0xe8e0cc, flatShading: true,
      }), 0.6));
      // Glowing top
      ...
      const top = new THREE.Mesh(new THREE.IcosahedronGeometry(0.9, 0), topMat);
    ...
    case 'altar': {
      // Low stone slab with dark crystal
      const slabG = new THREE.BoxGeometry(3.5, 0.6, 2.5);
      const slab = new THREE.Mesh(slabG, stoneMat);
      slab.position.y = 0.3;
      group.add(slab);
      const crystalMat = new THREE.MeshLambertMaterial({
        color: place.color, emissive: 0x4a1030, emissiveIntensity: 0.6, flatShading: true,
      });
      const crystal = new THREE.Mesh(new THREE.OctahedronGeometry(1.0, 0), crystalMat);
    ...
    case 'boss': {
      // Tall blood-red obelisk
      group.add(makeBase(3.5, 0.6, stoneMat));
      const bossMat = new THREE.MeshLambertMaterial({
        color: place.color, emissive: 0x4a0000, emissiveIntensity: 0.35, flatShading: true,
      });
...
// Helper constructors for place structures
...
function makeColumn(height, radius, mat, yOffset) {
  const g = new THREE.CylinderGeometry(radius * 0.85, radius, height, 8);
  const m = new THREE.Mesh(g, mat);
...
function buildTreesForChunk(cx, cz) {
  ...
  // Check if this chunk has a named place — reduce density near it
...
function setPlayerTarget(x, z) {
  // Clamp target to world bounds — no walking into the ocean
...
function updateRegionTracking() {
  ...
  if (region.name !== lastRegionName) {
    // Only show banner if it's a named place or a wilderness transition
