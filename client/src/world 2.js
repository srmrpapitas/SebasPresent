...
function animate() {
  ...
  if (character) {
    character.update(dt);
    // FIX flotar: raycast vertical hacia abajo desde el player para
    // detectar la altura REAL del terreno (que no es plano) y alinear
    // los pies del personaje con el suelo. Sin esto, Nico flota cuando
    // el terreno tiene altura variable bajo él.
    if (player && !characterFallback && terrainMeshes.length > 0) {
      _playerDownOrigin.set(player.position.x, player.position.y + 5, player.position.z);
      _playerDownRay.set(_playerDownOrigin, _playerDownDir);
      const groundHits = _playerDownRay.intersectObjects(terrainMeshes, false);
      if (groundHits.length > 0) {
        const groundY = groundHits[0].point.y;
        const bbox = new THREE.Box3().setFromObject(player);
        const feetY = bbox.min.y;
        const correction = groundY - feetY;
