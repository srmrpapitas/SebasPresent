...
function findGroundItemAtTap() {
  // Solo raycast directo al hitbox del item (no proximidad screen-space).
  // El hitbox es un mesh invisible más grande que el cubo visual, para
  // que en móvil sea fácil de acertar sin bloquear taps al suelo.
  const meshList = [];
  for (const item of groundItemsMap.values()) {
    if (item.hitMesh) meshList.push(item.hitMesh);
  }
