...
async function attemptChop(treeType, tx, tz) {
  try {
    ...
    // Sesión 32 — SFX hachazo en cada intento (logre log o no). El sonido
    // acompaña la anim de tala que YA está corriendo en loop. No depende
    // del éxito del roll de log porque visualmente el char SIEMPRE da el
    // hachazo (es la dinámica de OSRS).
    try {
      if (typeof window.__playSfx === 'function') {
        window.__playSfx('wc_chop');
      }
    } catch {}
