/**
 * SebasPresent — Auth flow
 *
 * Orchestrates login / register / logout by calling the API module
 * and updating the UI module. No DOM access here — clean separation.
 */

import * as api from './api.js';
import * as ui from './ui.js';

// ---------- Validation (client-side, server still validates) ----------
...
export async function handleLogin(event) {
  ...
  // Client-side guards — keeps roundtrips down for obvious errors.
...
function onAuthenticated(user) {
  ...
  if (ui.els.welcomeUsername) ui.els.welcomeUsername.textContent = user.username;
  if (ui.els.welcomeJoined) ui.els.welcomeJoined.textContent = ui.formatDate(user.created_at);
...
export async function tryResumeSession() {
  ...
  try {
    ...
    // Token expired or invalid — clear and fall through to login screen.
    api.clearToken();
    return false;
  } finally {
    ui.setLoading(false);
