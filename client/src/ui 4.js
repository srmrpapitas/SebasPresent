...
const els = {
  ...
  // Inventory elements are created dynamically by inventory.js (invGrid, invCount, invError)
};
export { els };
...
export function initSidebar({ onLogout } = {}) {
  ...
  // ---- Tab switching ----
  // We use `pointerup` instead of `click` because iOS does NOT fire click
  // events on a second finger while another touch (e.g. joystick) is active.
  // pointerup fires for every touch independently, so multi-touch works.
  ...
  const handleTabActivate = (btn) => {
    ...
    // If sidebar is minimized, tapping a tab restores it AND activates that tab
...
// Helper for setSidebarTab to trigger via pointerup-compatible path
export function setSidebarTab(name) {
  const sidebar = els.osrsSidebar;
  if (!sidebar) return;
  const btn = sidebar.querySelector(`[data-tab-btn="${name}"]`);
  if (!btn) return;
  // Dispatch a synthetic pointerup
