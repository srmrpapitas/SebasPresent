/**
 * SebasPresent — World module (Slice 3 — Remy + minimap + better joystick + GLB trees)
 *
  ...
  await Promise.all(entries.map(async ([typeId, url]) => {
    ...
    try {
      ...
      const parts = meshes.map(m => {
        ...
        // Make sure the material doesn't break in our lighting setup
        let material = m.material;
        if (Array.isArray(material)) material = material[0];
        // GLBs often come with PBR; we keep the existing material since
        // it carries the texture, but we make sure it's double-sided so
        // leaf alpha cards look right.
        if (material) {
          material.side = THREE.DoubleSide;
          if (material.transparent || material.alphaTest > 0) {
            material.alphaTest = 0.5;
            material.transparent = false; // alphaTest is enough; transparent kills depth
          }
        }
