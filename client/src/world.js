...
function animate() {
  ...
  if (character) {
    character.update(dt);
    // FIX flotar: bbox normalize CADA FRAME con threshold pequeño.
    // La animación cambia la altura del bbox; sin esto, el personaje
    // "flota" cuando la cámara está baja.
    if (player && !characterFallback) {
      const bbox = new THREE.Box3().setFromObject(player);
      if (Math.abs(bbox.min.y) > 0.02) {
