...
const PLAYER_RUN_BOOST = 1.6; // Multiplier when run mode is on
...
// Collision radius around trees (in world units, after scaling).
// Player can't walk inside this radius. 0.6 is roughly trunk thickness
// for a max-sized tree (3x scale).
const TREE_COLLISION_RADIUS = 0.6;

// Position auto-save interval (ms). Sends current position to server.
const POSITION_SAVE_INTERVAL = 10_000; // every 10s
// Minimum distance moved before triggering a save (avoids saving when
// the player is just standing still tapping things)
const POSITION_SAVE_MIN_DELTA = 5.0;

// Backend API base — same Worker that handles auth.
...
// Full-map modal state (the big OSRS-style world map)
let fullMapCanvas = null;
let fullMapCtx = null;
let fullMapOverlay = null;
let fullMapVisible = false;

// Run mode (boots toggle). When true, player moves at PLAYER_RUN * PLAYER_RUN_BOOST.
let runMode = false;

// HUD elements (HP, Prayer, Run)
let hudHpValue = null;
...
let authToken = null;            // injected from main.js / startWorld
let positionSaveTimer = 0;       // accumulator (in seconds)
let lastSavedX = 0;
let lastSavedZ = 0;

// Collision data — populated whenever a chunk is loaded. Map of
// `${cx},${cz}` → array of {x, z, radius} for fast lookup near the player.
...
export async function startWorld(loggedInUser, token) {
  ...
  try {
    ...
    // Restore last known position from server (if logged in).
    // If this fails, we just start at origin.
    if (authToken) {
...
export function stopWorld() {
  // Save position before tearing down (best-effort, fire-and-forget)
  if (running && player && authToken) {
    savePositionBeacon(player.position.x, player.position.z);
  }
  ...
  // Hide the minimap and full-map (don't remove — they live in the HTML)
  if (minimapCanvas) {
...
function loadChunk(cx, cz) {
  ...
  // Collect tree positions into the collider map for this chunk so we
  // can do fast collision checks against them in updatePlayer.
  // Each tree-trunk InstancedMesh has userData.trees (the array of
  // {x, z, typeId}). We use ONE collider per tree (not per part).
  const colliders = [];
  for (const m of treeMeshes) {
    ...
    if (!list) continue;
    for (const t of list) {
      // bush_small is too small to collide with, skip
...
function setupMinimap() {
  // The canvas lives in the HTML (id="worldMinimap") inside .osrs-minimap-wrap.
  // We just grab it, ensure visibility, give it OSRS styling, and attach
  // the click-to-open-fullmap handler.
  const el = document.getElementById('worldMinimap');
  if (!el) {
    console.warn('Minimap canvas not found in HTML');
    return;
  }
  ...
  // Tap minimap → open full-screen world map
  // pointerup so it works while joystick is active (multi-touch)
...
function drawMinimap() {
  ...
  // Range shown on the minimap, in world meters. Higher = more zoomed out.
  // OSRS style: shows a wide view with the player as a small dot in the
  // middle.
  const RANGE = 900;
  const cx = W / 2, cy = H / 2;
...
function setupFullMap() {
  ...
  if (!fullMapOverlay || !fullMapCanvas) {
    console.warn('Full map elements not found in HTML');
    return;
  }
  ...
  // Tap outside the frame (on the overlay backdrop) also closes.
  // Kept as 'click' on purpose: while the full map is open, the joystick
  // is paused, so multi-touch isn't a concern here.
  addL(fullMapOverlay, 'click', (e) => {
    if (e.target === fullMapOverlay) closeFullMap();
  });
}
...
// Renders the entire 4096×4096 world to the full-map canvas, with
// biomes as colored regions, places as labeled icons, and the player
// as a white dot. Static — drawn once per open (not per frame).
function drawFullMap() {
  if (!fullMapCtx || !player) return;
  const ctx = fullMapCtx;
  const W = fullMapCanvas.width;
  const H = fullMapCanvas.height;
  // World extent: ±WORLD_HALF → maps to [0, W] and [0, H]
  ...
  // Background — ocean
  ctx.fillStyle = '#4a7896';
  ctx.fillRect(0, 0, W, H);

  // Sample biome grid (coarse, every ~40 world units → 100 samples per side)
  const SAMPLES = 100;
  const cellW = W / SAMPLES;
  const cellH = H / SAMPLES;
  ...
  // Player position (white pulsing dot + name)
  ...
  ctx.fillStyle = grad;
  ctx.beginPath();
  ctx.arc(ps.x, ps.y, 14, 0, Math.PI * 2);
  ctx.fill();
  // Solid dot
  ctx.beginPath();
...
function setupHud() {
  ...
  // Initial values (placeholder until combat/prayer systems land)
  ...
  // Run toggle — tap the boot icon to run/walk
  // Uses pointerup (not click) so it works while the joystick is active.
  // iOS Safari does NOT fire 'click' on a second finger when another touch
  // is already active.
  if (hudStatRun) {
...
async function fetchPosition() {
  ...
  if (!res.ok) {
    console.warn('fetchPosition failed:', res.status);
    return null;
  }
...
// Synchronous beacon (used during logout / page unload). The browser
// guarantees this completes even after the page is gone.
function savePositionBeacon(x, z) {
  if (!authToken || !navigator.sendBeacon) return;
  try {
    // sendBeacon doesn't support custom headers — we use a Blob with
    // a custom MIME so the Worker can still parse it. But auth header
    // isn't possible here. Workaround: include token in the JSON body
    // and let the Worker also accept body-based tokens for /api/position.
    // SIMPLER ALTERNATIVE: just do a regular fetch with keepalive: true,
    // which DOES support headers and works for page unload.
    ...
    // Page might already be gone, can't do anything
  }
}
...
// Accumulate time and periodically save the player's current world
// position to the server. Only saves if the player has actually moved
// meaningfully since the last save (avoids spamming the API).
...
function updatePlayer(dt) {
  ...
  // Effective max speed depends on whether boots/run mode is on
  ...
  if (joyState.active && (Math.abs(joyState.x) > 0.15 || Math.abs(joyState.y) > 0.15)) {
    ...
    // Tree collision: if the new position would put us inside a tree
    // trunk, slide along the obstacle instead of going through it.
    ...
    if (dist < 0.1) {
      ...
      let nextX, nextZ;
      if (step >= dist) {
        nextX = playerTarget.x;
        nextZ = playerTarget.z;
      } else {
        const nx = dx / dist;
        const nz = dz / dist;
      ...
      // Collision check
      const adjusted = applyTreeCollision(player.position.x, player.position.z, nextX, nextZ);
      // If we got blocked (adjusted position roughly equal to current),
      // cancel the target — we can't reach there directly.
...
/**
 * Tree collision resolution. Given a current position (x0, z0) and a
 * desired next position (x1, z1), returns the final position after
 * resolving any tree collisions in the path.
 *
 * Strategy: try X and Z axes separately. If a tree blocks the X move,
 * cancel just that axis (so the player can still slide along Z, and
 * vice versa). This gives natural feeling sliding against tree trunks.
 */
function applyTreeCollision(x0, z0, x1, z1) {
  // Only check colliders near the player's current chunk + neighbors
  ...
  // Sanity check the combined: if even the partial moves still hit
  // (e.g. tree right at corner), don't move at all
  if (collidesWithTree(finalX, finalZ, cx, cz, r2)) {
    return { x: x0, z: z0 };
  }
...
function collidesWithTree(x, z, cx, cz, r2) {
  // Check current chunk + 8 neighbors
