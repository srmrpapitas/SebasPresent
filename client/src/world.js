...
function doCanvasTap(e) {
  ...
  // 0) Tap a item del suelo (loot) → ir a recogerlo
  const lootHit = findGroundItemNearTap(e.clientX, e.clientY);
  if (lootHit) {
...
const GROUND_ITEM_PICKUP_RADIUS_M = 3;     // distancia para poder recoger
const GROUND_ITEM_TAP_SCREEN_PX = 50;      // tolerancia de tap en pantalla

// Mapa id → { id, item_id, qty, x, z, name, group(THREE.Group), lastSeen }
...
function updateGroundItems(dt) {
  ...
  // 2) Pickup pendiente: si estamos andando hacia un item y ya estamos cerca,
  //    disparamos el pickup automáticamente.
  if (pendingPickupItemId !== null) {
    ...
    if (!item) {
      ...
      if (d <= GROUND_ITEM_PICKUP_RADIUS_M) {
        const idToPickup = pendingPickupItemId;
        pendingPickupItemId = null;
        pickupGroundItem(idToPickup);
      }
    }
  }

  // 3) Animación leve de bobbing (los items "flotan" un poco para verse)
...
function findGroundItemNearTap(clientX, clientY) {
  // Paso 1: raycast directo al mesh del item
  const meshList = [];
  for (const item of groundItemsMap.values()) {
    if (item.mesh) meshList.push(item.mesh);
  }
  if (meshList.length > 0) {
    const hits = raycaster.intersectObjects(meshList, false);
    if (hits.length > 0) {
      const dropId = hits[0].object.userData.itemDropId;
      const item = groundItemsMap.get(dropId);
      if (item) return item;
    }
  }
  // Paso 2: proximidad en pantalla (igual que NPCs)
  const rect = canvas.getBoundingClientRect();
  const localX = clientX - rect.left;
  const localY = clientY - rect.top;
  const tmpV = new THREE.Vector3();
  let best = null;
  let bestDist = GROUND_ITEM_TAP_SCREEN_PX;
  for (const item of groundItemsMap.values()) {
    if (!item.group) continue;
    tmpV.set(item.group.position.x, item.group.position.y + 0.2, item.group.position.z);
    tmpV.project(camera);
    if (tmpV.z > 1 || tmpV.z < -1) continue;
    const sx = (tmpV.x * 0.5 + 0.5) * rect.width;
    const sy = (-tmpV.y * 0.5 + 0.5) * rect.height;
...
function triggerGroundItemPickup(item) {
  ...
  const d = Math.hypot(dx, dz);
  if (d <= GROUND_ITEM_PICKUP_RADIUS_M) {
    // Cerca: pickup inmediato
    pickupGroundItem(item.id);
  } else {
    // Lejos: auto-walk hacia el item; updateGroundItems detecta la llegada
...
async function pickupGroundItem(itemDropId) {
  ...
  try {
    ...
    if (!r.ok) return;
    const data = await r.json();
    // El server devuelve picked_up y skipped. Quitamos los recogidos.
    const pickedUp = data?.picked_up || [];
    for (const pu of pickedUp) {
      const id = pu.id || pu;   // tolera ambos formatos
      removeGroundItem(id);
    }
