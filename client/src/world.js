...
const NPC_ENGAGE_RANGE = 2.0;  // distancia para auto-engage

const CAMERA_DIST_MIN = 6;
const CAMERA_DIST_MAX = 30;
const JOYCAM_YAW_SPEED = 2.0;    // rad/s
const JOYCAM_PITCH_SPEED = 1.2;  // rad/s
...
let joyCamState = { active: false, x: 0, y: 0 };
...
export function stopWorld() {
  ...
  joyCamState = { active: false, x: 0, y: 0 };
 ...
 */
function bakeGlbModel(root, targetHeight, fallbackColor) {
  if (detectsZUp(root)) {
...
async function loadGLBNpcs() {
  ...
  await Promise.all(entries.map(async ([typeId, url]) => {
    try {
      const gltf = await loader.loadAsync(url);
      const baked = bakeGlbModel(gltf.scene, NPC_TARGET_HEIGHTS[typeId] || 1.0, NPC_FALLBACK_COLORS[typeId] || 0x808080);
      if (!baked) return;
...
async function setupPlayer() {
  ...
  try {
    ...
    // FIX: personaje "vuela" sobre el suelo. Bbox normalize a y=0.
    try {
      const bbox = new THREE.Box3().setFromObject(characterGroup);
      const offsetY = bbox.min.y;
      if (Math.abs(offsetY) > 0.02) {
        characterGroup.position.y -= offsetY;
        console.log(`[player] bbox normalized: shifted by ${(-offsetY).toFixed(3)}`);
      }
    } catch (e) { console.warn('[player] bbox normalize failed:', e); }
  } catch (err) {
  ...
  if (npcMeshList.length > 0) {
    ...
    if (npcHits.length > 0) {
      ...
      if (npc) {
        ...
        // Marcar como pending engage. Si ya estamos cerca → engage directo.
        pendingEngageNpcId = npcId;
        if (dist <= NPC_ENGAGE_RANGE) {
          // Estamos al lado, atacar ya
          pendingEngageNpcId = null;
          combat.engageNpc(npcId);
        } else {
          // Caminar hacia el NPC; updatePlayer hará engage cuando llegue
...
/**
 * Joystick derecho: control de cámara. X=yaw, Y=pitch.
 * El valor se aplica en updateCamera con velocidad proporcional.
 */
function setupJoystickCamera() {
  const joyEl = document.getElementById('joystickCamera');
  const joyKnob = document.getElementById('joystickCameraKnob');
  if (!joyEl || !joyKnob) return;
  let centerX = 0, centerY = 0;
  const MAX_R = 42;
  function setKnob(dx, dy) { joyKnob.style.transform = `translate(${dx}px, ${dy}px)`; }
  ...
  function update(cx, cy) {
    ...
    if (len > MAX_R) { dx = dx / len * MAX_R; dy = dy / len * MAX_R; }
    setKnob(dx, dy);
    joyCamState.x = dx / MAX_R;
    joyCamState.y = dy / MAX_R;
  }
  function onMove(ev) {
    if (!joyCamState.active) return;
    ev.preventDefault();
    const t = ev.touches ? ev.touches[0] : ev;
    update(t.clientX, t.clientY);
  }
  function onEnd() { joyCamState.active = false; joyCamState.x = 0; joyCamState.y = 0; setKnob(0, 0); }
  addL(joyEl, 'touchstart', onStart, { passive: false });
  addL(joyEl, 'touchmove', onMove, { passive: false });
  addL(joyEl, 'touchend', onEnd);
  addL(joyEl, 'touchcancel', onEnd);
  addL(joyEl, 'mousedown', onStart);
  addL(joyEl, 'mousemove', onMove);
  addL(joyEl, 'mouseup', onEnd);
  addL(joyEl, 'mouseleave', onEnd);
}

/**
 * Pinch zoom + rotación con 2 dedos. Activo SOLO si los dedos están
 * fuera de los joysticks (evitamos conflicto).
 */
function setupTouchCamera() {
  ...
  function touchInsideJoystick(touch) {
    ...
    const joyR = document.getElementById('joystickCamera');
...
function animate() {
  ...
  if (character) {
    character.update(dt);
    // FIX: personaje "vuela" — bbox normalize periódico (cada 0.5s)
    // porque la animación cambia la altura del bbox y el normalize de
    // setupPlayer (que se hace una sola vez) deja de ser válido.
    if (player && !characterFallback) {
      const now = clock.getElapsedTime();
      if (now - lastPlayerYNormalize > 0.5) {
        lastPlayerYNormalize = now;
        const bbox = new THREE.Box3().setFromObject(player);
        if (Math.abs(bbox.min.y) > 0.05) {
          player.position.y -= bbox.min.y;
        }
      }
    }
  }
...
function updateCamera(dt) {
  // Aplicar joystick derecho a yaw/pitch
  if (joyCamState.active && (Math.abs(joyCamState.x) > 0.1 || Math.abs(joyCamState.y) > 0.1)) {
    cameraYaw -= joyCamState.x * JOYCAM_YAW_SPEED * dt;
    cameraPitch -= joyCamState.y * JOYCAM_PITCH_SPEED * dt;
    cameraPitch = Math.max(0.1, Math.min(1.3, cameraPitch));
  }
