...
export async function startWorld(loggedInUser, token) {
  ...
  try {
    ...
    // Sesión 17 — Damage splat + XP drops + Level up banner
    try {
      damageSplat.start({
        getPlayerWorldPos: () => player ? player.position : null,
        getCameraProjection: () => {
          if (!player || !camera) return null;
          // Proyectar la cabeza del player (top de su altura visual)
      ...
      // Hook global para que combat.js dispare XP drops y daño al player.
      // combat.js ya llama __worldSpawnHitsplat para NPCs; añadimos las
      // funcionalidades nuevas a través de hooks propios.
      ...
      // Level up banner: skills.onLevelUp ya está suscrito al jingle
      // (lo hace injectSkillsPanel). Añadimos aquí el banner.
      skills.onLevelUp((evt) => {
        damageSplat.spawnLevelUpBanner(evt.skillId, evt.newLevel);
      });
...
export function stopWorld() {
  ...
  // Sesión 18 — limpiar HP bar del player
  if (hpBarEl) { try { hpBarEl.remove(); } catch {} hpBarEl = null; }
  authToken = null;
  positionSaveTimer = 0;
  runMode = false;
...
function updateNameTag() {
  ...
  // Sesión 18 — HP bar doble cara sobre el player. Se posiciona justo
  // encima del nombre. Lee HP/maxHP de combat.getStateSnapshot().
  try { updatePlayerHpBar(sx, sy); } catch (e) { /* silencioso, no romper render */ }
}

// ============================================================
// Sesión 18 — HP bar doble cara estilo OSRS sobre el player
// ============================================================
// Verde (HP actual) + Rojo (HP perdido) en una sola barra horizontal.
// Visible siempre que el player esté en pantalla (igual que OSRS).
// Mismo estilo se aplicará a peers (multiplayer) cuando se integre.

let hpBarEl = null;

function ensurePlayerHpBar() {
  if (hpBarEl) return hpBarEl;
  // Estilos
  if (!document.getElementById('player-hpbar-styles')) {
    ...
    style.textContent = `
      .player-hpbar {
        ...
        background: #5a0e0e;       /* base roja (HP perdido) */
        overflow: hidden;
        display: flex;
      }
      .player-hpbar-fill {
  ...
  hpBarEl.id = 'playerHpBar';
  hpBarEl.className = 'player-hpbar';
  hpBarEl.innerHTML = '<div class="player-hpbar-fill" style="width:100%"></div>';
  (document.getElementById('worldScreen') || document.body).appendChild(hpBarEl);
  return hpBarEl;
}

function updatePlayerHpBar(playerScreenX, playerScreenY) {
  const bar = ensurePlayerHpBar();
  // Leer HP/HP_max desde combat. getStateSnapshot puede no existir si el
  // módulo de combat aún no ha cargado state — defensivo.
  let snap = null;
  ...
  // Posicionar 16px arriba del name tag (que ya está en (sx, sy)).
