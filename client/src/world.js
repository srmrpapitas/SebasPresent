...
// Es TAP si SE CUMPLE CUALQUIERA de estas dos reglas:
//   A) maxDist <= TAP_DRAG_THRESHOLD     (dedo apenas se movió)
//   B) duration <= TAP_QUICK_MS Y maxDist <= TAP_QUICK_DIST_PX
//      (fue un tap rápido aunque el dedo temblara un poco)
// En cualquier otro caso es DRAG → ya rotó la cámara durante el movimiento.
// Antes el threshold era 8 px y bastaba con cruzarlo una vez para
// matar el tap. En móvil 8 px es ridículamente poco.
const TAP_DRAG_THRESHOLD = 30;          // px
const TAP_QUICK_MS = 500;               // ms: tap "rápido" hasta medio segundo
const TAP_QUICK_DIST_PX = 100;          // px: distancia máx para tap rápido
...
const LONG_PRESS_MS = 500;            // ms. Subido de 320 — 320 era demasiado
                                      // corto: taps "normales" duraban más y
                                      // se interpretaban como long-press,
                                      // bloqueando el tap simple.
...
function onCanvasPointerUp(e) {
  ...
  // Es TAP si CUALQUIERA de estas reglas se cumple:
  //   A) El dedo casi no se movió (maxDist <= threshold)
  //   B) Fue un toque rápido y la distancia es razonable
  // Si ninguna se cumple, fue un drag para rotar cámara (ya rotada).
  const isTap = (maxDist <= TAP_DRAG_THRESHOLD) ||
                (duration <= TAP_QUICK_MS && maxDist <= TAP_QUICK_DIST_PX);
  if (isTap) doCanvasTap(e);
}
