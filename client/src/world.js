...
function updateNpcHpBars() {
  if (!camera) return;
  // Para que la HP bar mire siempre a cámara hace falta descontar la rotación
  // del padre (el NPC group rota con el patrol). Trabajamos en world space:
  //   localQuat = invParentWorldQuat * cameraWorldQuat
  ...
  for (const mesh of npcMeshes.values()) {
    ...
    tmpParentQ.invert();
    tmpResultQ.multiplyQuaternions(tmpParentQ, camera.quaternion);
...
const GROUND_ITEM_TAP_SCREEN_PX = 60;      // tolerancia de tap en pantalla
